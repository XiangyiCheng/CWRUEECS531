...
# I is the image. X, Y are the locations of the arrows. U, V are the x, y components of the arrows.
...
def estimate_flow(seq):
    H, W = seq['I1'].shape
    gridsz = 9
    wsz  = 21
    # x, y are the locations of the arrows.
    ...
    for i in range(0, H):
        for j in range(0, W):
            # gridX[i,j] is the x coordinate; gridY[i,j] is the y coordinate.
            v =  estimate_flow_at_xy(seq["I1"], seq["I2"],gridX[i,j], gridY[i,j], wsz)
            U[i, j] = v[0]
            V[i, j] = v[1]

    #U, V = estimate_flow(seq["I1"], seq["I2"], gridX, gridY, wsz)
    #x,y are the locations of the arrows. U, V are the x, y components of the arrows.
    #plotflow(seq["I1"], x, y, U, V, 3, 0.3)
    plotflow(seq["I1"], x, y, U, V, 3, 0.3)
